# ⚛️ Chapter 17: Neural Quantum States (NQS)
...
The goal of this project is to implement the core objective function of Neural Quantum States (NQS): calculating the **expected energy $\langle \hat{H} \rangle$** of a trial quantum state $\psi_{\boldsymbol{\theta}}(\mathbf{s})$. This is the quantity that is minimized to find the quantum system's ground state.

### Theory: Variational Principle and Local Energy

The **Variational Principle** states that for any normalized trial wavefunction $\psi_{\boldsymbol{\theta}}$ parameterized by $\boldsymbol{\theta}$, the calculated expected energy $E(\boldsymbol{\theta})$ will always be greater than or equal to the true ground state energy $E_0$:

$$E(\boldsymbol{\theta}) = \frac{\langle \psi_{\boldsymbol{\theta}} | \hat{H} | \psi_{\boldsymbol{\theta}} \rangle}{\langle \psi_{\boldsymbol{\theta}} | \psi_{\boldsymbol{\theta}} \rangle} \ge E_0$$
...
$$E_{\text{loc}}(\mathbf{s}) = \sum_{\mathbf{s}'} \frac{\langle \mathbf{s} | \hat{H} | \mathbf{s}' \rangle}{\psi_{\boldsymbol{\theta}}(\mathbf{s})} \psi_{\boldsymbol{\theta}}(\mathbf{s}')$$
...
The goal is to implement the core formula for the **stochastic gradient ($\nabla_{\boldsymbol{\theta}} E$)** of the expected energy. This demonstrates that the optimization rule for the quantum problem is a modified form of the familiar TD learning rule.
...
$$\nabla_{\boldsymbol{\theta}} E = 2 \text{Re} \left[ \left\langle (E_{\text{loc}}(\mathbf{s}) - \langle \hat{H} \rangle) \nabla_{\boldsymbol{\theta}} \ln \psi_{\boldsymbol{\theta}}(\mathbf{s}) \right\rangle_{\pi} \right]$$
...
2.  **Feature Vector Analogue ($\nabla_{\boldsymbol{\theta}} \ln \psi$):** The gradient of the log-wavefunction, which acts as the effective feature vector for the network state $\mathbf{s}$.

The factor $\nabla_{\boldsymbol{\theta}} \ln \psi$ is calculated efficiently using **Automatic Differentiation (AD)**. This gradient acts as the **quantum force** that steers the classical optimizer.
...
The goal is to numerically verify the **monotonic non-increasing** behavior of the expected energy $E(\boldsymbol{\theta})$ during the Variational Monte Carlo (VMC) optimization.

### Theory: Energy Dissipation and Stability

The VMC method is a deterministic optimization process in the parameter space $\boldsymbol{\theta}$. The gradient descent rule $\boldsymbol{\theta}_{t+1} = \boldsymbol{\theta}_t - \eta \nabla_{\boldsymbol{\theta}} E$ is designed to move toward the minimum energy.
