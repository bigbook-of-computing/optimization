...
The goal is to implement the core components of **Bayes' Theorem**—the **Prior ($P(\mathcal{\theta})$)**, the **Likelihood ($\mathcal{L}(\mathcal{\theta})$)**, and the resulting **Posterior ($P(\mathcal{\theta} | \mathcal{D})$)**—and visualize how the data (Likelihood) updates the initial belief (Prior).
...
$$P(\mathcal{\theta} | \mathcal{D}) = \frac{P(\mathcal{D} | \mathcal{\theta}) P(\mathcal{\theta})}{P(\mathcal{D})} \quad \text{or, more simply:} \quad P(\mathcal{\theta} | \mathcal{D}) \propto \mathcal{L}(\mathcal{\theta}) P(\mathcal{\theta})$$

  * **Prior ($P(\mathcal{\theta})$):** Represents our belief about the parameters $\mathcal{\theta}$ *before* seeing the data. We typically assume a **Gaussian Prior**.
  * **Likelihood ($\mathcal{L}(\mathcal{\theta})$):** Represents the probability of observing the data $\mathcal{D}$ for a given set of parameters $\mathcal{\theta}$. This is the information provided by the simulation data.
  * **Posterior ($P(\mathcal{\theta} | \mathcal{D})$):** The final, updated belief. In this simple Gaussian-Gaussian case, the posterior is also Gaussian (**conjugate prior** property). Its mean and variance are tighter than the prior, reflecting reduced uncertainty.
...
1.  **Maximum Likelihood (MLE):** The point estimate that maximizes the likelihood of the data $\mathcal{L}(\mathcal{\theta})$. For the Gaussian mean, $\hat{\mathcal{\theta}}_{\text{MLE}} = \text{Sample Mean}$.
2.  **Maximum A Posteriori (MAP):** The point estimate that maximizes the posterior $P(\mathcal{\theta} | \mathcal{D})$:
```
$$\hat{\mathcal{\theta}}_{\text{MAP}} = \arg\max_{\mathcal{\theta}} [\ln \mathcal{L}(\mathcal{\theta}) + \ln P(\mathcal{\theta})]$$

```
The term $\ln P(\mathcal{\theta})$ acts as a **regularization penalty**. A strong (narrow) prior effectively pulls the MAP estimate toward the prior mean, while a weak (wide) prior allows the MAP estimate to remain close to the MLE (the sample mean).
...
$$D_{\mathrm{KL}}(Q || P) = \int Q(\mathcal{\theta}) \ln \frac{Q(\mathcal{\theta})}{P(\mathcal{\theta})} d\mathcal{\theta}$$
