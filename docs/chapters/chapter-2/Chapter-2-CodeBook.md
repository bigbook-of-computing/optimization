# 📘 Chapter 2: Statistics & Probability in High Dimensions
...
The goal is to numerically find the **Maximum Likelihood Estimate (MLE)** for the mean ($\boldsymbol{\mu}$) and covariance ($\boldsymbol{\Sigma}$) of a 2D Gaussian distribution, verifying that the analytical MLE solution corresponds simply to the **sample mean** and **sample covariance**.

### Theory: MLE for the Gaussian

The MLE approach finds the parameters ($\boldsymbol{\mu}, \boldsymbol{\Sigma}$) that maximize the **log-likelihood** of observing the generated data:

$$\ln \mathcal{L}(\boldsymbol{\theta}) = \sum_{i=1}^N \ln p(\mathbf{x}_i|\boldsymbol{\theta})$$
...
$$\hat{\boldsymbol{\mu}}_{\text{MLE}} = \frac{1}{N}\sum_{i=1}^N \mathbf{x}_i \quad \text{and} \quad \hat{\Sigma}_{\text{MLE}} = \frac{1}{N}\sum_{i=1}^N (\mathbf{x}_i - \hat{\boldsymbol{\mu}})(\mathbf{x}_i - \hat{\boldsymbol{\mu}})^{\top}$$
