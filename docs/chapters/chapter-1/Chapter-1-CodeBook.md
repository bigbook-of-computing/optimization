...
The goal of this project is to implement the crucial data preprocessing step of **standardization (Z-score normalization)** and verify its effect on the dataset's features [cite: uploaded:Chapter-1-WorkBook.ipynb]. This is necessary because raw simulation outputs often have features (variables) with widely different physical units and magnitudes [cite: uploaded:Chapter-1.ipynb].

### Theory: Standardization and Feature Scaling

Standardization transforms the raw data matrix $X$ ($M$ samples $\times$ $D$ features) into a standardized matrix $X'$, ensuring that all features are placed on an equal footing for subsequent geometric analysis [cite: uploaded:Chapter-1.ipynb, uploaded:Chapter-1-WorkBook.ipynb].

The transformation is applied independently to each feature (column) $j$ in the data matrix [cite: uploaded:Chapter-1.ipynb]:
...
The successful application of standardization guarantees that the transformed data $X'$ will have a mean of approximately zero ($\boldsymbol{\mu}' \approx \mathbf{0}$) and a standard deviation of approximately one ($\boldsymbol{\sigma}' \approx \mathbf{1}$) for all features [cite: uploaded:Chapter-1-WorkBook.ipynb, uploaded:Chapter-1.ipynb].
...
|:-------------------|---------------:|----------------:|----------------:|-----------------:|
| Position_X (μ=100) |   100.001      |        0.999219 |     4.03769e-14 |                1 |
| Velocity_Y         |    -0.0172067  |        1.02925  |    -3.03091e-17 |                1 |
| Energy (σ^2=50)    |     0.0321231  |        7.10869  |     7.10543e-18 |                1 |
| Spin_Z             |     0.00653078 |        0.959842 |     6.78346e-17 |                1 |
| Temp_T             |     0.0328456  |        0.988835 |    -4.64073e-17 |                1 |

Conclusion: Standardization successfully transformed the data. The features with wildly different raw means (e.g., 100.0) and standard deviations (e.g., 7.0) now all have a mean of approximately 0.0 and a standard deviation of 1.0, ensuring that all features contribute equally to the final geometric analysis.

```

[image-tag: code-generated-image-0-1762818575067061942]
...
The goal of this project is to compute the **covariance matrix ($\boldsymbol{\Sigma}$)** for a synthetic dataset where correlations are manually engineered [cite: uploaded:Chapter-1-WorkBook.ipynb]. By interpreting the diagonal and off-diagonal elements, we demonstrate that the covariance matrix correctly encodes the underlying physical dependencies and geometric shape of the data cloud [cite: uploaded:Chapter-1.ipynb].

### Theory: Covariance as a Dependence Map

The covariance matrix $\boldsymbol{\Sigma}$ is a $D \times D$ matrix summarizing the **second moments** of the data [cite: uploaded:Chapter-1.ipynb].

$$\Sigma_{jk} = \frac{1}{M-1}\sum_{i=1}^M (x_{ij} - \mu_j)(x_{ik} - \mu_k)$$

  * **Diagonal Elements ($\boldsymbol{\Sigma_{ii}}$):** These are the **variances** of each individual feature, measuring its spread along its own axis.
  * **Off-Diagonal Elements ($\boldsymbol{\Sigma_{jk}, j \neq k}$):** These are the **covariances**, encoding the **linear correlation** between feature $j$ and feature $k$. A large magnitude (positive or negative) signifies that the two features are strongly dependent and move together [cite: uploaded:Chapter-1-WorkBook.ipynb, uploaded:Chapter-1.ipynb].

This matrix is the fundamental bridge between statistical data and geometric analysis, revealing the interconnectedness of the system's components [cite: uploaded:Chapter-1.ipynb].
...
[image-tag: code-generated-image-0-1762818598413849203]
...
The goal is to replicate the core **Principal Component Analysis (PCA)** visualization, which involves projecting high-dimensional, correlated data onto the subspace defined by the first two **principal axes** [cite: uploaded:Chapter-1-WorkBook.ipynb]. This visually confirms that PCA identifies the direction of maximum variance in the data cloud [cite: uploaded:Chapter-1.ipynb].

### Theory: PCA and Eigendecomposition

PCA is based on the **eigendecomposition of the covariance matrix ($\boldsymbol{\Sigma}$)** [cite: uploaded:Chapter-1.ipynb].

$$\Sigma \mathbf{v}_k = \lambda_k \mathbf{v}_k$$

  * **Eigenvectors ($\mathbf{v}_k$):** The principal axes, defining the orthogonal directions of greatest variance [cite: uploaded:Chapter-1.ipynb].
  * **Eigenvalues ($\lambda_k$):** The variance captured along each corresponding axis [cite: uploaded:Chapter-1.ipynb].

The transformation projects the mean-centered data $\mathbf{x}'$ onto the subspace spanned by $\mathbf{v}_1$ and $\mathbf{v}_2$ to create the 2D map (the "shadow" of the data cloud) [cite: uploaded:Chapter-1.ipynb, uploaded:Chapter-1-WorkBook.ipynb]. This linear projection is the first step in discovering the system's low-dimensional **manifold** ($\mathcal{M}$) [cite: uploaded:Chapter-1.ipynb].
...
[image-tag: code-generated-image-0-1762818624272984660]
...
The goal is to quantitatively determine the **effective intrinsic dimensionality** of a high-dimensional system by analyzing the **explained variance ratio ($\lambda_k / \sum \lambda_j$)** of the eigenvalues ($\lambda_k$) from PCA [cite: uploaded:Chapter-1-WorkBook.ipynb].
...
The **intrinsic dimensionality** ($d$) is determined by the number of components required to capture a high percentage of the total variance (e.g., 95% or 99%) [cite: uploaded:Chapter-1-WorkBook.ipynb]. This number of components $d$ approximates the true dimensionality of the data's underlying **manifold** ($\mathcal{M}$), confirming that physical systems are highly constrained [cite: uploaded:Chapter-1.ipynb, uploaded:Chapter-1-WorkBook.ipynb].
...
```text?code_stdout&code_event_index=15
...
[image-tag: code-generated-image-0-1762818648799199069]
