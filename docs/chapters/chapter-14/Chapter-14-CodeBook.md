...
The $\boldsymbol{\beta}$ factor explicitly controls the trade-off, allowing us to enforce a simpler structure ($\beta > 1$) at the expense of potential image quality (Energy).
...
The **$\boldsymbol{\beta}$ factor** explicitly controls the trade-off, allowing the user to enforce a simpler latent structure ($\beta > 1$) at the expense of potential image quality (Energy/Reconstruction).
