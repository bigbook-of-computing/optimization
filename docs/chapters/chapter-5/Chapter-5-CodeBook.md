# Chapter 5: Gradient Methods: The Workhorses

## Project 1: Visualizing Learning Rate and Dynamics ($\boldsymbol{L(\theta) = \frac{1}{2}\theta^2}$)
...
Gradient Descent is the fundamental law of motion for optimization, analogous to an **overdamped physical system** where velocity ($\propto \Delta\boldsymbol{\theta}$) is proportional to the force ($\propto -\nabla L$).
...
$$\frac{dL}{dt} = \frac{\partial L}{\partial \boldsymbol{\theta}} \cdot \frac{d\boldsymbol{\theta}}{dt}$$

Since the motion is defined by the negative gradient, $\frac{d\boldsymbol{\theta}}{dt} = -\eta \nabla L$, the change in loss is:
