...
Policy Gradient methods directly optimize the policy $\pi_{\mathcal{\theta}}(\mathbf{a} \mid \mathbf{s})$ parameterized by $\mathcal{\theta}$. The gradient of the objective function (expected return $J$) is estimated using the **likelihood ratio trick**:

$$\nabla_{\mathcal{\theta}} J \propto \sum_{t} \nabla_{\mathcal{\theta}} \ln \pi_{\mathcal{\theta}}(\mathbf{a}_t|\mathbf{s}_t) \cdot A_t$$
...
The **$\mathcal{\alpha}$ parameter** (analogue of inverse temperature $\alpha \sim 1/T$) controls the balance:
